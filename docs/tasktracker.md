...
## Задача: Расширение поддержки AstraLinux на все основные дистрибутивы
- **Статус**: Завершена
- **Описание**: Расширить поддержку AstraLinux для всех основных релизов и добавить совместимый offline-mapping.

### Шаг 1. Анализ текущей поддержки AstraLinux
- **Статус**: Завершена
- **Описание**:
  - Выявлено, что в `linux_supported` Astra ограничен диапазоном `1..1`.
  - В `packages_dirs.deb` присутствовал только `Astra Linux 1.7_x86-64 -> astra17`.

### Шаг 2. Реализация расширения
- **Статус**: Завершена
- **Описание**:
  - В [vars/main.yml](../vars/main.yml) расширен supported-range: `Astra Linux 1..2`.
  - Добавлены offline mapping записи для релизов AstraLinux:
    - `1.5/1.5_x86-64 -> debian9`,
    - `1.6/1.6_x86-64 -> debian9`,
    - `1.7/1.7_x86-64 -> astra17`,
    - `1.8/1.8_x86-64 -> debian12`,
    - `2.12/2.12_x86-64 -> debian9`.

### Шаг 3. Документация и валидация
- **Статус**: Завершена
- **Описание**:
  - Актуализированы [README.md](../README.md), [docs/CONFIGURATION.md](./CONFIGURATION.md), [docs/PROJECT_ANALYSIS.md](./PROJECT_ANALYSIS.md), [CHANGELOG.md](../CHANGELOG.md), [docs/changelog.md](./changelog.md).
  - Проверено наличие новых mapping-записей в `vars/main.yml`.

### Текущий статус
- **Общий статус задачи**: Завершена
- **Оставшиеся шаги**:
  - По запросу: добавить native AstraLinux offline bundles под каждый релиз вместо compatibility-модели.
